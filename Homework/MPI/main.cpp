...
vector<vector<int>> optimalOpenKnightMPI(int N, int M, stretchedType stretchedType, int avaibleNodes, int parentNode);
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < rowsToReceive; ++i) {
        ...
        if (rowsReceived[i][2] == 0 && rowsReceived[i][3] == 0) {
            temp = optimalOpenKnight(rowsReceived[i][0], rowsReceived[i][1], noStretched);
            if (temp.empty()) {
            ...
            if (rowsReceived[i][2] == 0) {
                temp = optimalOpenKnight(rowsReceived[i][0], rowsReceived[i][1], anyStretched);
            }
            else if (rowsReceived[i][2] == 2) {
                temp = optimalOpenKnight(rowsReceived[i][0], rowsReceived[i][1], verticalStretched);
            }
            else {
                temp = optimalOpenKnight(rowsReceived[i][0], rowsReceived[i][1], horizontalStretched);
            }
            if (temp.empty()) {
                cout << "rank " << rank << " : No solution" << endl;
                MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
